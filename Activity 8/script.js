//inicializa el array de canciones vazio. esto es necesario para poder referenciarlo fuera del initialize
let songs = [];

// función arrow initialize para inicializar todo (será ejecutada al final de la página)
var initialize = () => {
    // declaración de variables y manipulación de DOM necesarias para las funciones inicializadas
...
// función convoluta que seleciona la numeración de la canción actual (por medio de la captación del número de la ruta img) y actualiza para la próxima canción en el array de objetos
...
var nextSong = () => {
    ...
    const path = mainSong.src;
    const mainSongStr = path.split("/")[5];
    const nextSongPos = parseInt(mainSongStr.slice(0, 1));
...
// función convoluta que seleciona la numeración de la canción actual (por medio de la captación del número de la ruta img) y actualiza para la canción anterior en el array de objetos
